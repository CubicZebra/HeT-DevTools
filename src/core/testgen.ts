...
export function renderTestFile(moduleName: string, headerInclude: string, apis: ApiFunction[]): string {
  ...
  for (const api of apis) {
    ...
    if (api.paramTypes.length === 0) {
      ...
      emit(`${api.name}_positive`, [
        '// TODO(het): 补充参数与期望后启用真实断言',
        `GTEST_SKIP() << "参数类型待人工确认: ${api.paramTypes.join(', ')}";`,
        `(void)${api.name}; // 引用符号，保证链接`,
      ]);
    }
    ...
    if (api.paramTypes.length === 0) {
      emit(`${api.name}_boundary_reentrant`, [
        ...
        `ASSERT_NO_THROW(${api.name}()); // 重复调用不应崩溃`,
      ]);
    } else if (callable && api.paramTypes.length === 1) {
      const k = kinds[0];
      const edge =
        k === 'int' ? '-1 /* 边界：负数 */' : k === 'uint' ? '0u /* 边界：零 */' : k === 'pointer' || k === 'cstring' ? 'nullptr' : defaultValue(k);
      emit(`${api.name}_boundary`, [
        '// TODO(het): 按实现契约补充边界期望',
    ...
    emit(`${api.name}_negative`, [
      '// TODO(het): 断言错误输入被拒绝或抛出（语义由实现契约决定）',
  ...
  return [
    ...
    `// 可补充断言与更多用例；main.cpp 由系统维护，请勿手改。`,
    ``,
    sections,
    ``,
  ].join('\n');
