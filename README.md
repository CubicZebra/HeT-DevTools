...
点击活动栏左侧的 **HeT DevTools** 图标 →「进入驾驶舱」，或在命令面板执行 `HeT DevTools: 打开驾驶舱`。

- **11 页单页导航**：概览 / 构建与测试 / 依赖 / 模块与测试 / 文档 / 质量 / 提交 / 发布 / 上板 / 协作与审计 / 设置，每页实时数据来自工程本身。
- **情境化抽屉**：构建/测试运行时底部日志抽屉自动展开流式输出，成功后 3 秒自动收起；失败自动展开“问题”抽屉并映射到问题面板。
- **依赖页深表单**：驾驶舱内直接增删依赖（四桶分组，确认后原子双写 `conandata.yml` + `metadata.json`）。
- **上板页粘贴解析**：无硬件时粘贴 `BENCHMARK_START / RESULT|case|value / BENCHMARK_END` 模拟串口输出，即时解析结果表。
- **五步新项目向导**：空工作区自动弹出（顶栏「＋ 新项目」可随时重开）：模板源 → 身份 → 构建参数 → 开关 → 确认，一键从模板落地完整工程。
- **模板红点**：顶栏「●模板可更新 N」提示本地模板前进的提交数。
- 最后页面与向导草稿自动持久化（重启恢复）；顶栏/抽屉/向导支持 zh/en 切换；深浅主题全适配。
